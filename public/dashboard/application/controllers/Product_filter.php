...
class Product_filter extends CI_Controller {
	...
	function fetch_data()
	{
		...
		$config = array();
		$config["base_url"] = "#";
		$config["total_rows"] = $this->product_filter_model->count_all($minimum_price, $maximum_price, $brand, $style, $size,$shape);
		$config["per_page"] = 100;
		...
		$output = array(
			...
			'product_list'			=>	$this->product_filter_model->fetch_data($config["per_page"], $start, $minimum_price, $maximum_price, $brand, $style, $size,$shape)
		);
